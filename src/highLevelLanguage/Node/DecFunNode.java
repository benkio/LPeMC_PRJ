...
public class DecFunNode extends Node {
	...
	public String toPrint() {
		...
		+ funName
		+ "</FunName>"
		+ (hasGeneric ? "<FunGenericType>"
				+ funGenericsToPrint + "</FunGenericType>" : "")
	...
	public String typeCheck() {
		...
		if (!typeChecked) {
			...
			if(this.hasGeneric && !this.genericInst) {
				//TODO dioporco
				return"";
			}
				...
				for(int cont=0; cont< this.funParams.size(); cont++){
					if (funParams.get(cont).getType().getNodeType()== NodeType.GENERICTYPE_NODE){
						for(int cont2=0; cont2< this.generics.size();cont2++){
							...
							if(((GenericTypeNode)funParams.get(cont).getType()).getId() == generics.get(cont2).getId()){
								notfound=false;
							}
							if(notfound){
								//TODO dioporco
